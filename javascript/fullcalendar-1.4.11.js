	...
	function renderBasic(r, c, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					if (j == 0) {
                        workweek = "<span class='rk-workweek' style='float:left;'>v" + d.getISOWeek() + "</span>"; 
                     }
                    else {
                        workweek = "";
                    }
			...
			tbody.find('td').each(function() {
				...
				td.find('span.rk-workweek').text("v" + d.getISOWeek());
				addDays(d, 1);
				if (nwe) {
					skipWeekend(d);
				}
		...
		if (!head) { // first time rendering, build from scratch
			...
			s = "<div class='fc-agenda-head' style='position:relative;z-index:4'>" +
				...
				"<th class='fc-leftmost " +
					tm + "-state-default'><span class='rk-workweek' style='float:left;'>v" + d.getISOWeek() + "</span></th>";
			for (i=0; i<colCnt; i++) {
				s += "<th class='fc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default' +
			...
			for (i=0; d < maxd; i++) {
				...
				s += "<tr class='" +
					(!i ? 'fc-first ' : '') + (!minutes ? 'fc-major' : 'fc-minor') +
			...
			head.find('span.rk-workweek').text("v" + d0.getISOWeek());
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		var resourceCnt = 0;
		for (i = 0; i < segCnt; i++) {
			if (segs[i].event.id && $.inArray(segs[i].event.id, resources) == -1) {
				resources.push(segs[i].event.id);
				resourceLevels[segs[i].event.id] = resourceCnt;
				resourceCnt++;
			}
		}
